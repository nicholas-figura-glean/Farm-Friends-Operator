...
#   com.nickfigura.farmfriends.outage     the 300s confirmed-outage Slack notifier
#   com.nickfigura.farmfriends.eod        the 17:00 MT Farm Friends sundown report
...
OUTAGE_PLIST="$AGENT_DIR/$OUTAGE.plist"
EOD_PLIST="$AGENT_DIR/$EOD.plist"
...
if [[ "${1:-}" == "--uninstall" ]]; then
  ...
  launchctl bootout "$DOMAIN/$OUTAGE" 2>/dev/null || true
  launchctl bootout "$DOMAIN/$EOD" 2>/dev/null || true
  ...
  rm -f "$PLIST" "$SUPERVISOR_PLIST" "$EXPAND_PLIST" "$RECOVERY_PLIST" \
        "$OUTAGE_PLIST" "$EOD_PLIST" "$CONTRACT_PLIST" "$AUTHOR_PLIST" \
        "$RESEARCH_PLIST" "$DASHBOARD_PLIST" "$MONITOR_PLIST"
...
for label in "$LABEL" "$SUPERVISOR" "$EXPAND" "$RECOVERY" "$OUTAGE" "$EOD" "$CONTRACT" "$AUTHOR" "$RESEARCH" "$DASHBOARD" "$MONITOR"; do
...
echo "installed $OUTAGE -> every 300s (locally-cleared confirmed outage notifications)"
echo "installed $EOD -> daily at 17:00 local Mountain Time (Farm Friends sundown report)"
