...
by evidence. Baselines and holds live in ``state/meta.json`` so a restart cannot
...
def event_signature(text: str) -> str:
    ...
    routine = (
        ...
        (r"\b(?:laid|produced|made honey|found a truffle|gave milk|grew wool)\b", "production"),
...
def assess(
    ...
    # Evidence-linked question closure is the only automatic unblock path.
    blocks = {
        key: dict(value)
        for key, value in (current.get("blocks") or {}).items()
        if not _settled(dict(value), questions)
    }
